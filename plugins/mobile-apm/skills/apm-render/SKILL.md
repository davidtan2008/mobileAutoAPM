...
**必读**：`${CLAUDE_PLUGIN_ROOT}/references/metrics-definitions.md` §2、§3。
...
python3 "${CLAUDE_PLUGIN_ROOT}/scripts/apm_screenshot.py" \
...
```bash
python3 "${CLAUDE_PLUGIN_ROOT}/scripts/apm_white_screen.py" \
  .apm/runs/<本次>/screenshots/*.png

# 机器可读
python3 "${CLAUDE_PLUGIN_ROOT}/scripts/apm_white_screen.py" --json shot.png
```
