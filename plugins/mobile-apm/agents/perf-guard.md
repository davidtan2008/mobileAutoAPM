...
- CV > 30% 或诊断发现多峰时**必须**报告"测量不可信，结论无效"，而不是照样给判定。
- 劣化项必须**列在最前面**，不要埋在表格里。
